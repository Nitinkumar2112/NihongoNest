...
class N4GrammarViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun loadAllProgress() {
        val grammarTracker = createTracker("n4_grammar_main_progress", "JLPT N4 Grammar", getN4GrammarLessons())
        val practiceTracker = createTracker("n4_grammar_practice_progress", "Grammar Practice", getN4GrammarPracticeLessons())
        val oneShotTracker = createTracker("n4_grammar_oneshot_progress", "1-Shot Grammar (before exam)", getN4OneShotLessons())
